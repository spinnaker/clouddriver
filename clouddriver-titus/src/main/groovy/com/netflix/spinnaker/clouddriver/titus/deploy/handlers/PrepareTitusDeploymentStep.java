...
public class PrepareTitusDeploymentStep extends AbstractTitusDeployStep
  ...
  private static final Logger log = LoggerFactory.getLogger(PrepareTitusDeploymentStep.class);
  ...
  public List<SagaEvent> apply(@NotNull Front50AppLoaded event, @NotNull Saga saga) {
    ...
    if (!isNullOrEmpty(asgName)) {
      ...
      mergeSourceDetailsIntoDescription(saga, description, front50Application);
    } else {
  ...
  @Override
  public void compensate(@NotNull Front50AppLoaded event, @NotNull Saga saga) {
