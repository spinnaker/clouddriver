...
class OpenstackSubnetProvider implements SubnetProvider<OpenstackSubnet> {
  ...
  OpenstackSubnet fromCacheData(CacheData cacheData) {
    Subnet subnet = objectMapper.convertValue(cacheData.attributes, NeutronSubnet)
    ...
    new OpenstackSubnet(
      ...
      allocationPools: subnet.allocationPools?.collect { it -> new OpenstackSubnet.Range(it.start, it.end) },
