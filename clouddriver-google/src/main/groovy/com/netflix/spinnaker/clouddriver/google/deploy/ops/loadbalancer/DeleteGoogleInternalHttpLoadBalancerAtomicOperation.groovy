...
@Slf4j
class DeleteGoogleInternalHttpLoadBalancerAtomicOperation extends DeleteGoogleLoadBalancerAtomicOperation {
  ...
  DeleteGoogleInternalHttpLoadBalancerAtomicOperation(DeleteGoogleLoadBalancerDescription description) {
  ...
  Void operate(List priorOutputs) {
    task.updateStatus BASE_PHASE, "Initializing deletion of Internal HTTP load balancer $description.loadBalancerName..."
    ...
    if (!forwardingRule) {
      GCEUtil.updateStatusAndThrowNotFoundException("Frwarding rule $forwardingRuleName not found in $region for $project",
          task, BASE_PHASE)
    }
    ...
    def retrievedTargetProxy = GCEUtil.getRegionTargetProxyFromRule(compute, project, region, forwardingRule, BASE_PHASE, safeRetry, this)

    if (!retrievedTargetProxy) {
      GCEUtil.updateStatusAndThrowNotFoundException("Target proxy $targetProxyName not found for $project in $region", task,
          BASE_PHASE)
    }
    ...
    projectForwardingRules
      .each { ForwardingRule rule ->
        if (rule.getLoadBalancingScheme() != "INTERNAL_MANAGED") return
        try {
        def proxy = GCEUtil.getRegionTargetProxyFromRule(compute, project, region, rule, BASE_PHASE, safeRetry, this)
    ...
    for (String backendServiceUrl : backendServiceUrls) {
      ...
      BackendService backendService = safeRetry.doRetry(
        ...
        [400, 403, 412],
        [],
        [action: "get", phase: BASE_PHASE, operation: "compute.backendServices.get", (TAG_SCOPE): SCOPE_REGIONAL, (TAG_REGION): region],
        registry
      ) as BackendService
      if (backendService?.backends) {
        task.updateStatus BASE_PHASE, "Server groups still associated with Internal Http(s) load balancer ${description.loadBalancerName}. Failing..."
        throw new IllegalStateException("Server groups still associated with Internal Http(s) load balancer: ${description.loadBalancerName}.")
      }
    ...
    listenersToDelete.each { String ruleName ->
      task.updateStatus BASE_PHASE, "Deleting listener $ruleName in $region..."
      Operation operation = GCEUtil.deleteRegionalListener(compute, project, region, ruleName, BASE_PHASE, safeRetry, this)
    ...
    task.updateStatus BASE_PHASE, "Deleting URL map $urlMapName in $region..."
    Operation deleteUrlMapOperation = safeRetry.doRetry(
      ...
      [400, 403, 412],
      [404],
      [action: "delete", phase: BASE_PHASE, operation: "compute.regionUrlMaps.delete", (TAG_SCOPE): SCOPE_REGIONAL, (TAG_REGION): region],
      registry
    ) as Operation

    googleOperationPoller.waitForRegionalOperation(compute, project, region, deleteUrlMapOperation.getName(),
        timeoutSeconds, task, "Regional url map " + urlMapName, BASE_PHASE)
    ...
    for (BackendServiceAsyncDeleteOperation asyncOperation : deleteBackendServiceAsyncOperations) {
      googleOperationPoller.waitForRegionalOperation(compute, project, region, asyncOperation.operationName,
          timeoutSeconds, task, "Region backend service " + asyncOperation.backendServiceName, BASE_PHASE)
    }
    ...
    if (description.deleteHealthChecks) {
      ...
      for (HealthCheckAsyncDeleteOperation asyncOperation : deleteHealthCheckAsyncOperations) {
        googleOperationPoller.waitForRegionalOperation(compute, project, region, asyncOperation.operationName,
            timeoutSeconds, task, "region health check " + asyncOperation.healthCheckName,
            BASE_PHASE)
      }
    }

    task.updateStatus BASE_PHASE, "Done deleting internal http load balancer $description.loadBalancerName in $region."
    null
  }
}
