...
class LoadBalancerV2UpsertHandler {
  ...
  private static String modifyTargetGroupAttributes(AmazonElasticLoadBalancing loadBalancing, TargetGroup targetGroup, UpsertAmazonLoadBalancerV2Description.Attributes attributes) {
    ...
    if (attributes) {
      if (attributes.deregistrationDelay) {
        targetGroupAttributes.add(new TargetGroupAttribute(key: "deregistration_delay.time", value: attributes.deregistrationDelay.toString()))
      }
  ...
  public static void updateLoadBalancer(AmazonElasticLoadBalancing loadBalancing, LoadBalancer loadBalancer,
    ...
    def existingTargetGroups = loadBalancing.describeTargetGroups(new DescribeTargetGroupsRequest().withLoadBalancerArn(loadBalancerArn))?.targetGroups
    ...
    if (listeners) {
      ...
      listenersToCreate.each { listener ->
        try {
          ...
          task.updateStatus BASE_PHASE, "Listener added to ${loadBalancerName} (${listener.port}:${listener.protocol})."
        } catch (AmazonServiceException e) {
          def exceptionMessage = "Failed to add listener to ${loadBalancerName} (${listener.port}:${listener.protocol}) - reason: ${e.errorMessage}."
          task.updateStatus BASE_PHASE, exceptionMessage
          amazonErrors << exceptionMessage
        }
      }
