...
public class AssumeRoleAmazonCredentials extends AmazonCredentials {
    ...
    public AssumeRoleAmazonCredentials(@JsonProperty("name") String name,
                                       ...
                                       @JsonProperty("maxSecurityGroupsPerInstance") Integer maxSecurityGroupsPerInstance,
                                       @JsonProperty("assumeRole") String assumeRole,
                                       @JsonProperty("sessionName") String sessionName) {
        this(name, environment, accountType, accountId, defaultKeyPair, regions, defaultSecurityGroups, requiredGroupMembership, lifecycleHooks, maxSecurityGroupsPerInstance, null, assumeRole, sessionName);
    }

    public AssumeRoleAmazonCredentials(AssumeRoleAmazonCredentials copy, AWSCredentialsProvider credentialsProvider) {
        this(copy.getName(), copy.getEnvironment(), copy.getAccountType(), copy.getAccountId(), copy.getDefaultKeyPair(), copy.getRegions(), copy.getDefaultSecurityGroups(), copy.getRequiredGroupMembership(), copy.getLifecycleHooks(), copy.getMaxSecurityGroupsPerInstance(), credentialsProvider, copy.getAssumeRole(), copy.getSessionName());
    }

    AssumeRoleAmazonCredentials(String name, String environment, String accountType, String accountId, String defaultKeyPair, List<AWSRegion> regions, List<String> defaultSecurityGroups, List<String> requiredGroupMembership, List<LifecycleHook> lifecycleHooks, Integer maxSecurityGroupsPerInstance, AWSCredentialsProvider credentialsProvider, String assumeRole, String sessionName) {
        super(name, environment, accountType, accountId, defaultKeyPair, regions, defaultSecurityGroups, requiredGroupMembership, lifecycleHooks, maxSecurityGroupsPerInstance, createSTSCredentialsProvider(credentialsProvider, accountId, assumeRole, sessionName == null ? DEFAULT_SESSION_NAME : sessionName));
