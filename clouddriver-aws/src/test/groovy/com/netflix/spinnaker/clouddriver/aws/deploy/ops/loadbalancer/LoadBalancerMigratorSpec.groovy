...
class LoadBalancerMigratorSpec extends Specification {
  ...
  def setup() {
    ...
    strategy = new DefaultMigrateLoadBalancerStrategy(amazonClientProvider, regionScopedProviderFactory, securityGroupStrategy, deployDefaults)
  ...
  void 'getTargetSecurityGroups maps security group IDs to actual security groups'() {
    ...
    when:
    def targets = strategy.getTargetSecurityGroups(sourceLookup, targetLookup, sourceDescription, source, target)
    ...
    sourceLookup.getSecurityGroupById('test', 'sg-1', 'vpc-1') >> sourceUpdater1
    sourceLookup.getSecurityGroupById('test', 'sg-2', 'vpc-1') >> sourceUpdater2
  }
  ...
  void 'creates elb group and adds classic link ingress when moving from non-VPC to VPC'() {
    ...
    when:
    strategy.generateResults(sourceLookup, targetLookup, source, target, 'internal', 'app', false)
    ...
    1 * targetLookup.getSecurityGroupByName(prodCredentials.name, 'classic-link', 'vpc-2') >> new SecurityGroupUpdater(elbGroup, amazonEC2)
