...
public class CreateServerGroupForExistingServiceSpec extends EcsSpec {
  ...
  @DisplayName(
      ".\n===\n"
          + "Given description w/ task def artifacts, EC2 launch type, and new target group "
          + "fields with the existing service, successfully submit createServerGroup operation"
          + "\n===")
  @Test
  public void createServerGroup_ArtifactsEC2TgMappingsExistingServiceTest()
    ...
    String requestBody =
        generateStringFromTestFile(
            "/createServerGroup-artifact-EC2-targetGroupMappings-existingService.json");
    String expectedServerGroupName =
        "ecs-integArtifactsEC2TgMappingsExistingServiceStack-detailTest-v001";
    ...
    ByteArrayInputStream byteArrayInputStreamOfArtifactsForEC2Type =
        new ByteArrayInputStream(
            generateStringFromTestArtifactFile(
                    "/createServerGroup-artifact-EC2-targetGroupMappings-existingService-artifactFile.json")
                .getBytes());
    ...
    when(mockECS.listServices(any(ListServicesRequest.class)))
        .thenReturn(
            new ListServicesResult()
                .withServiceArns(
                    Collections.singletonList(
                        "arn:aws:ecs:ecs-integArtifactsEC2TgMappingsExistingServiceStack-detailTest")));
    ...
    assertEquals(
        "integArtifactEC2TgMappingsExistingService-cluster", seenCreateServRequest.getCluster());
  }
  ...
  public void createServerGroup_inputsEC2TgMappingsExistingServiceTest()
    ...
    String expectedServerGroupName =
        "ecs-integInputEC2TgMappingsExistingServiceStack-detailTest-v001";
    ...
    when(mockECS.listServices(any(ListServicesRequest.class)))
        .thenReturn(
            new ListServicesResult()
                .withServiceArns(
                    Collections.singletonList(
                        "arn:aws:ecs:ecs-integInputEC2TgMappingsExistingServiceStack-detailTest")));
