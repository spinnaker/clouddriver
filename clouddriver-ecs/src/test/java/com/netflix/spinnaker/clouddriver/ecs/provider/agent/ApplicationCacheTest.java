...
public class ApplicationCacheTest extends CommonCachingAgent {
  ...
  public void shouldRetrieveFromWrittenCache() {
    ...
    when(providerCache.filterIdentifiers(
            ECS_APPLICATIONS.toString(), "ecs;services;test-account;us-west-2;*"))
    ...
    when(providerCache.get(ECS_APPLICATIONS.toString(), key))
        .thenReturn(
            cacheResult.getCacheResults().get(ECS_APPLICATIONS.toString()).iterator().next());
    Application retrievedApplication = client.get(key);

    // Then
    Collection<CacheData> cacheData =
        cacheResult.getCacheResults().get(ECS_APPLICATIONS.toString());
