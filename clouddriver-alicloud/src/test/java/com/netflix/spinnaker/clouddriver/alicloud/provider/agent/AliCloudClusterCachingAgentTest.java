...
public class AliCloudClusterCachingAgentTest extends CommonCachingAgentTest {
  ...
  public void testBefore() throws ClientException {
    when(client.getAcsResponse(any()))
        .thenAnswer(new CommonAnswer());
  }

  @Test
  public void testLoadData() throws ClientException {
    ...
    assertTrue(instances.stream().anyMatch(i->i.getId().equals(instanceKey)));

    assertEquals(1, loadBalancers.size());
    assertEquals(loadBalancerKey, loadBalancers.iterator().next().getId());
