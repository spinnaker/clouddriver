...
public class AliCloudLoadBalancerCachingAgent implements CachingAgent, AccountAware, OnDemandAgent {
  ...
  public CacheResult loadData(ProviderCache providerCache) {
    ...
    if (queryResponse == null || queryResponse.getLoadBalancers()==null || queryResponse.getLoadBalancers().isEmpty()) {
  ...
  private DefaultCacheResult buildCacheResult(DescribeLoadBalancersResponse queryResponse) {
    List defaultCacheDatas =  queryResponse.getLoadBalancers().stream()
        .map(this::getLoadBalancerDetail)
        .map(detail -> {
          LoadBalancer loadBalancer = detail.loadBalancer;
          Map<String, Object> map = objectMapper.convertValue(loadBalancer, Map.class);
          map.put("account", account.getName());
          Map<String, Object> attributeMap = objectMapper.convertValue(detail.loadBalancerAttribute, Map.class);
          attributeMap.put("listenerPortsAndProtocal", detail.listenerAttributes);

          map.put("attributes", attributeMap);
          map.put("vServerGroups", detail.vServerGroups);

          return new DefaultCacheData(
              Keys.getLoadBalancerKey(
                  loadBalancer.getLoadBalancerName(), account.getName(), region, loadBalancer.getVpcId()),
              map,
              Maps.newHashMap());
        })
  ...
  @NotNull
  private LoadBalancerDetail getLoadBalancerDetail(LoadBalancer q) {
    String loadBalancerId = q.getLoadBalancerId();

    LoadBalancerDetail
        loadBalancerDetail = new LoadBalancerDetail();
    loadBalancerDetail.loadBalancer = q;
    ...
    try {
      loadBalancerDetail.loadBalancerAttribute =
          client.getAcsResponse(describeLoadBalancerAttributeRequest);
      List<ListenerPortAndProtocal> listenerPortsAndProtocal = loadBalancerDetail.loadBalancerAttribute
          .getListenerPortsAndProtocal();

      loadBalancerDetail.listenerAttributes = listenerPortsAndProtocal.stream()
          .map(portAndProtocal -> describeLoadBalancerListenerAttribute(loadBalancerId,portAndProtocal))
    ...
    try {
      ...
      DescribeVServerGroupsResponse describeVServerGroupsResponse = client.getAcsResponse(describeVServerGroupsRequest);
      loadBalancerDetail.vServerGroups = describeVServerGroupsResponse.getVServerGroups();
    } catch (ClientException e) {
      e.printStackTrace();
    }
  ...
  private Map<String,Object> describeLoadBalancerListenerAttribute(String loadBalancerId,ListenerPortAndProtocal portAndProtocal){
  ...
  @Data
  public static class LoadBalancerDetail{
