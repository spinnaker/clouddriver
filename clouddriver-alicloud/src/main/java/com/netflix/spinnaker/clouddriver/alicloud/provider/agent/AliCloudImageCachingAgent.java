...
public class AliCloudImageCachingAgent implements CachingAgent, AccountAware {
  ...
  public CacheResult loadData(ProviderCache providerCache) {
    ...
    List<CacheData> imageDatas =
        listMaps.stream()
            .map(
                map ->
                    new DefaultCacheData(
                        Keys.getImageKey(
                            String.valueOf(map.get("imageId")), account.getName(), region),
                        map,
                        new HashMap<>(16)))
            .collect(Collectors.toList());

    List<CacheData> nameImageDatas =
        listMaps.stream()
            .map(
                map ->
                    new DefaultCacheData(
                        Keys.getNamedImageKey(
                            account.getName(), String.valueOf(map.get("imageName"))),
                        map,
                        new HashMap<>(16)))
            .collect(Collectors.toList());
