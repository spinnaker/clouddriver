    ...
    public BasicAliCloudDeployDescription convertDescription(Map input) {
        ...
        if (vServerGroups != null) {
            ...
            for (Map map : vServerGroups) {
                CreateScalingGroupRequest.VServerGroup vServerGroup = getObjectMapper().convertValue(map,
                    CreateScalingGroupRequest.VServerGroup.class);
                ArrayList<Map> vServerGroupAttributes = (ArrayList<Map>)map.getOrDefault("vserverGroupAttributes",
                    map.get("vServerGroupAttributes"));
                ...
                if(vServerGroupAttributesList.stream().noneMatch(v->StringUtils.isBlank(v.getVServerGroupId()))){
