...
public class TencentInstanceProvider implements InstanceProvider<TencentInstance, String> {
  ...
  public TencentInstance getInstance(final String account, final String region, String id) {
    ...
    CacheData cacheData = cacheView.getAll(INSTANCES.ns, new ArrayList<String>(Arrays.asList(key)),
      RelationshipCacheFilter.include(LOAD_BALANCERS.ns, SERVER_GROUPS.ns, CLUSTERS.ns)).stream()
  ...
  public TencentInstance instanceFromCacheData(String account, String region, CacheData cacheData) {
    TencentInstance instance = objectMapper.convertValue(cacheData.getAttributes().get("instance"), TencentInstance.class);

    String serverGroupName = instance.getServerGroupName();
    CacheData serverGroupCache = cacheView.get(SERVER_GROUPS.ns, Keys.getServerGroupKey(serverGroupName, account, region));
    final Map<String, Object> attributes = (serverGroupCache == null ? null : serverGroupCache.getAttributes());
    Map asgInfo = (attributes == null ? null : (Map) attributes.get("asg"));
    List<Map<String, Object>> lbInfos = asgInfo == null ? null : (List<Map<String, Object>>) asgInfo.get("forwardLoadBalancerSet");
    if (!CollectionUtils.isEmpty(lbInfos)) {
      for (Map<String, Object> lbInfo : lbInfos) {
        ...
        String lbHealthKey = Keys.getTargetHealthKey(lbId, listenerId, "", instance.getName(), account, region);
        CacheData lbHealthCache = cacheView.get(HEALTH_CHECKS.ns, lbHealthKey);
        final Map<String, Object> healthAttributres = (lbHealthCache == null ? null : lbHealthCache.getAttributes());
        TencentLoadBalancerTargetHealth loadBalancerTargetHealth = healthAttributres == null ? null : (TencentLoadBalancerTargetHealth) healthAttributres.get("targetHealth");
        if (loadBalancerTargetHealth != null) {
          TencentTargetHealth targetHealth = new TencentTargetHealth(loadBalancerTargetHealth.getHealthStatus());
          TencentTargetHealth.TargetHealthStatus healthStatus = targetHealth.getTargetHealthStatus();
