...
public class TencentLoadBalancerListener {
  ...
  public void copyListener(TencentLoadBalancerListener listener) {
    ...
    this.targets =
        listener.targets.stream()
            .map(
                it -> {
                  TencentLoadBalancerTarget.TencentLoadBalancerTargetBuilder targetBuilder =
        ...
        listener.rules.stream()
            .map(
                it -> {
                  ...
                  rule.setTargets(
                      it.getTargets().stream()
                          .map(
                              t -> {
                                TencentLoadBalancerTarget.TencentLoadBalancerTargetBuilder target =
                                    TencentLoadBalancerTarget.builder();
