...
class AzureSecurityGroupResourceTemplate {
  ...
  static class SecurityGroup extends DependingResource{
    ...
    SecurityGroup(UpsertAzureSecurityGroupDescription description) {
      ...
      tags = ["appName":description.appName, "detail":description.detail]
  ...
  static class  AzureNSGSecurityRules {
    ArrayList<AzureNSGRule> securityRules = []

    AzureNSGSecurityRules(List<AzureSGRule> rules) {
      rules.each { rule -> securityRules.add(new AzureNSGRule(rule))}
    }
  }
