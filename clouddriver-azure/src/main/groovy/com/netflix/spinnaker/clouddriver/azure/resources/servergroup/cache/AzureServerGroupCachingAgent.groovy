...
class AzureServerGroupCachingAgent extends AzureCachingAgent {
  ...
  CacheResult loadData(ProviderCache providerCache) {
    ...
    buildCacheResult(providerCache, creds.computeClient.getServerGroupsAll(region), currentTime, null, null)
  }
  ...
  CacheResult buildCacheResult(ProviderCache providerCache,
    ...
    if (serverGroups) {
      ...
      Collection<String> identifiers = providerCache.filterIdentifiers(Keys.Namespace.AZURE_ON_DEMAND.ns, Keys.getServerGroupKey(azureCloudProvider, "*", region, accountName))
      ...
      serverGroups.each { AzureServerGroupDescription serverGroup ->
        ...
        if (sg) {
          ...
          cachedApplications[appKey].with {
            ...
            relationships[Keys.Namespace.AZURE_CLUSTERS.ns].add(clusterKey)
          }
          ...
          cachedClusters[clusterKey].with {
            ...
            relationships[Keys.Namespace.AZURE_APPLICATIONS.ns].add(appKey)
            relationships[Keys.Namespace.AZURE_SERVER_GROUPS.ns].add(serverGroupKey)
          }
      ...
      return new DefaultCacheResult([
        (Keys.Namespace.AZURE_APPLICATIONS.ns) : cachedApplications.values(),
        (Keys.Namespace.AZURE_CLUSTERS.ns) : cachedClusters.values(),
        (Keys.Namespace.AZURE_SERVER_GROUPS.ns) : cachedServerGroups.values()
      ], [(Keys.Namespace.AZURE_ON_DEMAND.ns): evictions as List<String>])
      ...
      if (updatedServerGroup) {
        if (updateCache(providerCache, updatedServerGroup, ON_DEMAND_UPDATED)) {
          ...
          return new DefaultCacheResult([(Keys.Namespace.AZURE_SERVER_GROUPS.ns): [cachedServerGroup]])
        } else {
          return null
        }
      }
      ...
      if (evictedServerGroup) {
        if (updateCache(providerCache, evictedServerGroup, ON_DEMAND_EVICTED)) {
          ...
          return new DefaultCacheResult([(Keys.Namespace.AZURE_SERVER_GROUPS.ns): []])
        }
        else {
          return null
        }
    ...
    new DefaultCacheResult([(Keys.Namespace.AZURE_SERVER_GROUPS.ns): []])
  }

  @Override
  Set<AgentDataType> initializeTypes() {
    Collections.unmodifiableSet([
      AUTHORITATIVE.forType(Keys.Namespace.AZURE_SERVER_GROUPS.ns),
      INFORMATIVE.forType(Keys.Namespace.AZURE_APPLICATIONS.ns),
      INFORMATIVE.forType(Keys.Namespace.AZURE_CLUSTERS.ns)
    ] as Set)
  }
  ...
  OnDemandAgent.OnDemandResult handle(ProviderCache providerCache, Map<String, ? extends Object> data) {
    ...
    Map<String, Collection<String>> evictions = evictedServerGroup ? [(Keys.Namespace.AZURE_SERVER_GROUPS.ns): [getServerGroupKey(evictedServerGroup)]] : [:]
  ...
  private Boolean updateCache(ProviderCache providerCache, AzureServerGroupDescription serverGroup, String onDemandCacheType) {
    ...
    if (serverGroup) {
      ...
      def cacheResults = providerCache.getAll(Keys.Namespace.AZURE_ON_DEMAND.ns, [getServerGroupKey(serverGroup)])
