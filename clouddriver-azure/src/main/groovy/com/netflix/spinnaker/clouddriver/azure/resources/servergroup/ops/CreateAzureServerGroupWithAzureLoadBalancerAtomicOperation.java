...
class CreateAzureServerGroupWithAzureLoadBalancerAtomicOperation implements AtomicOperation<Map> {
  ...
  public Map operate(List priorOutputs) {
    ...
    try {
      ...
      AzureVirtualNetworkDescription vnetDescription = networkProvider.get(description.getAccountName(), description.getRegion(), description.getVnetResourceGroup(), virtualNetworkName);
      ...
      Optional<AzureSubnetDescription> filteredSubnet = subnets.stream()
        .filter(subnet -> subnet.getName().equals(subnetName))
        .findFirst();
      ...
      if (description.getCredentials().getUseSshPublicKey() != null && description.getCredentials().getUseSshPublicKey()) {
        templateParameters.put(
