...
class AzureSecurityGroupProvider implements SecurityGroupProvider<AzureSecurityGroup> {
  ...
  AzureSecurityGroup fromCacheData(boolean includeRules, CacheData cacheData) {
    ...
    List<Rule> inboundRules = includeRules ? buildInboundRules(sg) : []
    List<Rule> outboundRules = includeRules ? buildOutboundRules(sg) : []
  ...
  private static List<Rule> buildInboundRules(AzureSecurityGroupDescription sg) {
    List<Rule> rangeRules = []

    rangeRules.sort()
  }

  private static List<Rule> buildOutboundRules(AzureSecurityGroupDescription sg) {
    List<Rule> rangeRules = []
