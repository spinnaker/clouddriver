...
class CatsSearchProvider implements SearchProvider {
  ...
  @Autowired List<KeyParser> keyParsers;
    ...
    toQuery.each { String cache ->
      matches.addAll(cacheView.filterIdentifiers(cache, "*:${cache}:*${normalizedWord}*").findAll { String key ->
        try {
          ...
          // TODO: make cloudProvider a separate field instead of having it as part of the filters map
          ...
          if (parser) {
            ...
            return filters
              .entrySet().every { filter ->
              def vals = filter.value.split(',')
