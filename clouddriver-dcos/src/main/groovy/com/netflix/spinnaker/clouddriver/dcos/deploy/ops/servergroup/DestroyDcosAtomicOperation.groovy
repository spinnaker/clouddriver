...
class DestroyDcosAtomicOperation implements AtomicOperation<Void> {
  ...
  private final DcosServerGroupDescription description

  DestroyDcosAtomicOperation(DcosServerGroupDescription description) {
    this.description = description
  }

  /**
   * curl -X POST -H "Content-Type: application/json" -d '[ { "destroyServerGroup": { "serverGroupName": "kub-test-v000", "namespace": "default", "credentials": "my-dcos-account" }} ]' localhost:7002/dcos/ops
   */
  @Override
  Void operate(List priorOutputs) {
    ...
    if (credentials.apiAdaptor.getAutoscaler(namespace, description.serverGroupName)) {
      ...
      if (!credentials.apiAdaptor.deleteAutoscaler(namespace, description.serverGroupName)) {
        throw new DcosOperationException("Failed to delete associated autoscaler $description.serverGroupName in $namespace.")
      }
    }
    ...
    if (deployment && replicaSet) {
      ...
      if (credentials.apiAdaptor.getDeploymentRevision(deployment) == credentials.apiAdaptor.getDeploymentRevision(replicaSet)) {
        ...
        if (!credentials.apiAdaptor.deleteDeployment(namespace, deploymentName)) {
          throw new DcosOperationException("Failed to delete deployment ${deploymentName} in $namespace")
        } else {
    ...
    if (credentials.apiAdaptor.getReplicationController(namespace, description.serverGroupName)) {
      if (!credentials.apiAdaptor.hardDestroyReplicationController(namespace, description.serverGroupName)) {
        throw new DcosOperationException("Failed to delete $description.serverGroupName in $namespace.")
      }
    } else if (replicaSet) {
      ...
      throw new DcosOperationException("Failed to find replication controller or replica set $description in $namespace.")
    }
