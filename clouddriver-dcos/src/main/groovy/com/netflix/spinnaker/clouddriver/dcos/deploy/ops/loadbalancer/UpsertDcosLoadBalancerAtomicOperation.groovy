...
import com.netflix.spinnaker.clouddriver.dcos.deploy.description.loadbalancer.DcosNamedServicePort
import com.netflix.spinnaker.clouddriver.orchestration.AtomicOperation
import io.fabric8.dcos.api.model.ServiceBuilder
import io.fabric8.dcos.api.model.ServicePort
...
class UpsertDcosLoadBalancerAtomicOperation implements AtomicOperation<Map> {
  ...
  DcosLoadBalancerDescription description

  UpsertDcosLoadBalancerAtomicOperation(DcosLoadBalancerDescription description) {
  ...
  /*
   * curl -X POST -H "Content-Type: application/json" -d  '[ {  "upsertLoadBalancer": { "name": "kub-lb",  "ports": [ { "name": "http", "port": 80, "targetPort": 9376 } ], "account":  "my-dcos-account" } } ]' localhost:7002/dcos/ops
  */
  @Override
  Map operate(List priorOutputs) {
    ...
    serviceBuilder = serviceBuilder.addToSelector(DcosUtil.loadBalancerKey(name), 'true')
    ...
    for (ServicePort port : existingService?.spec?.ports) {
      def namedPort = new DcosNamedServicePort()
