...
class DeleteDcosLoadBalancerAtomicOperation implements AtomicOperation<Void> {
  ...
  DeleteDcosLoadBalancerAtomicOperation(DeleteDcosLoadBalancerAtomicOperationDescription description) {
  ...
  /*
   * curl -X POST -H "Content-Type: application/json" -d  '[ {  "deleteLoadBalancer": { "loadBalancerName": "kub-lb", "namespace": "default", "account":  "my-dcos-account" } } ]' localhost:7002/dcos/ops
  */
  @Override
  Void operate(List priorOutputs) {
    ...
    if (!credentials.apiAdaptor.deleteService(namespace, description.loadBalancerName)) {
      throw new DcosOperationException("Failed to delete service $description.loadBalancerName in $namespace")
    }
